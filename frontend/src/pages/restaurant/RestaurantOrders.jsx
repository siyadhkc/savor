    ...
    if (loading) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center bg-white">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Syncing production manifest...</p>
            </div>
        )
    }
            ...
            <div className="relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 italic">Kitchen Ops Dashboard</span>
                        </div>
                        <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">Production <span className="text-primary-600 font-light italic">Queue</span></h1>
                        <p className="text-slate-500 font-medium mt-2">Manage your incoming culinary requests and fulfillments.</p>
                    </div>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
                    ...
                    <div className="flex items-center gap-3 text-slate-400 text-[10px] font-black uppercase tracking-widest bg-white/50 px-4 py-2 rounded-xl border border-slate-100">
                        <Filter size={14} /> 
                        <span>{filteredOrders.length} Active Protocols</span>
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse min-w-[1000px]">
                            <thead>
                                <tr className="bg-slate-50/50">
                                    ...
                                    <th className="px-8 py-5 text-slate-400 font-black text-[10px] uppercase tracking-widest border-b border-slate-100 text-center">Kitchen Status</th>
                                    ...
                                    <th className="px-8 py-5 text-slate-400 font-black text-[10px] uppercase tracking-widest border-b border-slate-100 text-right pr-12">Change Status</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <Fragment key={order.id}>
                                        ...
                                        <AnimatePresence>
                                            {expandedOrders.includes(order.id) && (
                                                <tr>
                                                    <td colSpan="6" className="p-0 border-b border-transparent">
                                                        <motion.div
                                                                ...
                                                                <div className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100 flex flex-col">
                                                                    <div className="flex items-center justify-between mb-8">
                                                                        <div className="flex items-center gap-3">
                                                                            ...
                                                                            <div>
                                                                                <h4 className="text-lg font-black text-slate-900 tracking-tight leading-none mb-1">Formulation Manifest</h4>
                                                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Kitchen-Ready Specifications</p>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                        ...
                                                                        {order.items.map((item, idx) => (
                                                                            <div key={idx} className="flex justify-between items-center group/item hover:translate-x-1 transition-transform duration-300">
                                                                                <div className="flex gap-4 items-center">
                                                                                    <div className="w-10 h-10 flex items-center justify-center bg-slate-50 rounded-xl text-xs font-black text-slate-600 border border-slate-100/50">
                                                                                        {item.quantity}×
                                                                                    </div>
                                                                                    <div className="flex flex-col">
                                                                                        <span className="text-sm font-black text-slate-800 leading-none mb-1">{item.menu_item_name}</span>
                                                                                        <span className="text-[10px] font-bold text-primary-500/60 uppercase tracking-widest italic">Inventory Point</span>
                                                                                    </div>
                                                                                </div>
                                                                            </div>
                                                                        ))}
                                                                    ...
                                                                    <div className="mt-auto pt-6 border-t border-slate-50 flex justify-between items-end">
                                                                        <div>
                                                                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Total Compensation</p>
                                                                             <div className="flex items-center gap-3">
                                                                                <span className="text-4xl font-black text-slate-900 tracking-tighter leading-none">₹{order.total_amount}</span>
                                                                                <div className="px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded text-[9px] font-black uppercase tracking-widest">Confirmed</div>
                                                                             </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div className="space-y-6 flex flex-col justify-between">
                                                                    <div className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100">
                                                                        <div className="flex items-center gap-3 mb-8">
                                                                            ...
                                                                            <div>
                                                                                ...
                                                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Dispatch Baseline</p>
                                                                            </div>
                                                                        </div>
                                                                        <div className="space-y-6">
                                                                            <div className="flex justify-between items-start">
                                                                                <div className="flex flex-col">
                                                                                    <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-2"><Clock size={12} className="text-primary-500" /> Intake Time</span>
                                                                            ...
                                                                            <div className="flex flex-col pt-4 border-t border-slate-50">
                                                                                <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><User size={12} className="text-primary-500" /> Customer Identity</span>
                                                                                <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                                                                                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">{order.user_email}</span>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div className="bg-slate-900 rounded-[32px] p-8 shadow-xl shadow-slate-900/10 text-white flex flex-col">
                                                                        <div className="flex items-center gap-3 mb-4">
                                                                            ...
                                                                            <span className="text-[10px] font-black uppercase tracking-widest text-white/50">Logistical Destination</span>
                                                                        </div>
                        ...
                        {filteredOrders.length === 0 && (
                            ...
                            >
                                ...
                                <h3 className="text-xl font-black text-slate-900 tracking-tight mb-1">Queue Empty</h3>
                                <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">No order entities requiring immediate protocol.</p>
                            </motion.div>
                        )}
                    </div>
                </div>
